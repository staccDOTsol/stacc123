...
const DEFAULT = ENDPOINTS[Math.floor(Math.random() * ENDPOINTS.length)].endpoint;
...
const ConnectionContext = React.createContext<ConnectionConfig>({
  ...
  env: ENDPOINTS[Math.floor(Math.random() * ENDPOINTS.length)].name,
  tokens: [],
...
export function ConnectionProvider({ children = undefined as any }) {
  ...
  const [savedEndpoint, setEndpoint] = useLocalStorageState(
    'connectionEndpoint',
    ENDPOINTS[Math.floor(Math.random() * ENDPOINTS.length)].endpoint,
  );
  ...
  const env =
    ENDPOINTS.find(end => end.endpoint === endpoint)?.name || ENDPOINTS[Math.floor(Math.random() * ENDPOINTS.length)].name;
